...
PREINSTALLED = {
    ...
    "empyrical-reloaded", "empyrical", "alphalens-reloaded", "alphalens",
    "arch", "cvxpy", "quantstats", "matplotlib", "plotly",
}
...
ALLOWLIST = PREINSTALLED | {
    "vectorbt", "backtrader", "pyportfolioopt", "riskfolio-lib", "riskparityportfolio",
    "ffn", "pandas-ta", "ta", "yfinance", "pandas-datareader", "pykalman",
    "hmmlearn", "linearmodels", "cvxopt", "numba", "seaborn", "networkx",
    "scikit-optimize", "hurst", "tslearn", "pmdarima", "filterpy", "quandl",
    "empyrical-reloaded", "alphalens-reloaded", "jquantstats",
}
...
class ProvisionResult:
    def __init__(self, image: str, installed: list[str], skipped: list[str],
...
def provision(requirements: list[str], timeout: int = 300) -> ProvisionResult:
    ...
    try:
        p = subprocess.run(
            ["docker", "build", "-t", tag, "-"],
            input=dockerfile, capture_output=True, text=True, timeout=timeout,
        )
    except subprocess.TimeoutExpired:
        return ProvisionResult(BASE_IMAGE, [], skipped, cached=False,
                               error=f"provision timed out installing {wanted}")
    if p.returncode != 0:
        return ProvisionResult(BASE_IMAGE, [], skipped, cached=False,
                               error=(p.stderr or p.stdout)[-400:])
    return ProvisionResult(tag, wanted, skipped, cached=False)


def _image_exists(tag: str) -> bool:
    try:
        p = subprocess.run(["docker", "image", "inspect", tag],
